...
def restart_services():
    ...
    subprocess.Popen(["celery", "-A", "progress_rpg", "worker", "--loglevel=debug"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # ✅ Restart the worker
...
def check_logs():
    ...
    subprocess.run("journalctl -u progress_rpg --no-pager | tail -n 50", shell=True, check=True)
