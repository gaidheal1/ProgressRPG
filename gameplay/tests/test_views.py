...
class SubmitActivityTest(GameplayViewTestBase):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.activity = Activity.objects.create(profile=cls.profile, name="Initial Activity")
    ...
    def test_submit_activity_invalid_json(self):
        response = self.client.post(reverse('submit_activity'), data="notjson", content_type="application/json")
        self.assertEqual(response.status_code, 400)
...
class ChooseQuestTest(GameplayViewTestBase):
    ...
    def test_choose_quest_invalid_content_type(self):
        response = self.client.post(reverse('choose_quest'), {"quest_id": self.quest.id})
        self.assertEqual(response.status_code, 400)
