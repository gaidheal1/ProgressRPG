...
def fetch_activities(request):
    ...
    try:
        ...
        logger.error(f"[FETCH ACTIVITIES] Serializer validation error: {str(e)}", exc_info=True)
...
def fetch_quests(request):
    ...
    try:
        ...
        logger.error(f"[FETCH QUESTS] Database operational error: {str(e)}", exc_info=True)
        ...
        return JsonResponse({"error": "An unexpected error occurred while fetching quests."}, status=500)
    ...
    try:
        ...
        if qt.time_finished() and qt.status != "complete":
            try:
                ...
                qt.save()
                logger.debug(f"[FETCH INFO] qt status {qt.status}, {qt.quest}, elapsed/remaining {qt.get_elapsed_time()}/{qt.get_remaining_time()}, duration {qt.duration}")
                ...
                logger.error(f"[FETCH INFO] Error handling quest timer completion for character {character.id}: {str(e)}", exc_info=True)
                return JsonResponse({"error": "An error occurred while handling quest timer completion."}, status=500)
        
        if profile.activity_timer.status != "empty" and profile.activity_timer.activity is None:
            try:
                logger.warning(f"[FETCH INFO] Timer status is {profile.activity_timer.status} but activity empty ({profile.activity_timer.activity}). Resetting activity timer")
                profile.activity_timer.reset()
            except Exception as e:
                # Handle errors related to resetting the activity timer.
                logger.error(f"[FETCH INFO] Error resetting activity timer for profile {profile.id}: {str(e)}", exc_info=True)
                return JsonResponse({"error": "An error occurred while resetting the activity timer."}, status=500)
        ...
        try:
            ...
            logger.error(f"[FETCH INFO] Error generating response for profile {profile.id}: {str(e)}", exc_info=True)
            return JsonResponse({"error": "An unexpected error occurred while generating the response."}, status=500)
        ...
        logger.error(f"[FETCH INFO] Database operational error: {str(e)}", exc_info=True)
        return JsonResponse({"error": "Database error occurred. Please try again later."}, status=500)
    except DatabaseError as e:
        ...
        return JsonResponse({"error": "An unexpected error occurred while fetching information."}, status=500)
...
def create_activity(request):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[CREATE ACTIVITY] Database error while creating activity for profile {profile.id}: {e}")
            return JsonResponse({"error": "A database error occurred while creating the activity."}, status=500)
        ...
        try:
            ...
            logger.error(f"[CREATE ACTIVITY] Validation error while serializing activity timer for profile {profile.id}: {e}")
            return JsonResponse({"error": "Invalid data encountered during serialization."}, status=400)

    except ObjectDoesNotExist as e:
        # Handle missing objects (e.g., related Profile or Timer not found)
        logger.error(f"[CREATE ACTIVITY] Object not found for profile {profile.id}: {e}")
        return JsonResponse({"error": "Required object not found."}, status=404)
    except OperationalError as e:
        # Handle broader database operational errors
        logger.error(f"[CREATE ACTIVITY] Database operational error for profile {profile.id}: {e}")
        return JsonResponse({"error": "A database error occurred. Please try again later."}, status=500)
    except Exception as e:
...
def submit_activity(request):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"[SUBMIT ACTIVITY] After activity submission and reset: status: {profile.activity_timer.status}, elapsed_time: {profile.activity_timer.elapsed_time}, XP reward: {xp_reward}")
        except DatabaseError as e:
            logger.error(f"[SUBMIT ACTIVITY] Database error while submitting activity for profile {profile.id}: {e}")
            return JsonResponse({"error": "A database error occurred while submitting activity."}, status=500)
        ...
        try:
            ...
            logger.error(f"[SUBMIT ACTIVITY] Object not found while fetching activities for profile {profile.id}: {e}")
        ...
        try:
            ...
            logger.debug(f"[SUBMIT ACTIVITY] Response generated for profile {profile.id}: {response}")
            return JsonResponse(response)
        except ValidationError as e:
            logger.error(f"[SUBMIT ACTIVITY] Validation error while serializing response for profile {profile.id}: {e}")
            return JsonResponse({"error": "Invalid data encountered during response serialization."}, status=400)

    except ObjectDoesNotExist as e:
        # Handle cases where `profile` or other objects are not found
        logger.error(f"[SUBMIT ACTIVITY] Object not found for profile {profile.id}: {e}")
        return JsonResponse({"error": "Required object not found."}, status=404)
    except OperationalError as e:
        # Handle broader database operational errors
        logger.error(f"[SUBMIT ACTIVITY] Database operational error for profile {profile.id}: {e}")
        return JsonResponse({"error": "A database error occurred. Please try again later."}, status=500)
    except Exception as e:
        # Catch-all for any unexpected errors
        logger.error(f"[SUBMIT ACTIVITY] Unexpected error for profile {profile.id}: {e}", exc_info=True)
        return JsonResponse({"error": "An unexpected error occurred while submitting activity"}, status=500)
...
def choose_quest(request):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"[CHOOSE QUEST] Quest {quest.name} (ID: {quest.id}) selected by user {profile.id}")
        except DatabaseError as e:
            logger.error(f"[CHOOSE QUEST] Database error while changing quest timer for user {profile.id}: {e}")
            return JsonResponse({"error": "A database error occurred while selecting the quest."}, status=500)
        ...
        # Handle broader database operational errors
        logger.error(f"[CHOOSE QUEST] Database operational error for user {profile.id}: {e}")
        return JsonResponse({"error": "A database error occurred. Please try again later."}, status=500)
    except Exception as e:
...
def complete_quest(request):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[COMPLETE QUEST] Database error while completing quest for character {character.id}: {str(e)}", exc_info=True)
            return JsonResponse({"error": "A database error occurred while completing the quest."}, status=500)
        except Exception as e:
        ...
        try:
            ...
            logger.error(f"[COMPLETE QUEST] Object not found while checking eligible quests for profile {profile.id}: {str(e)}", exc_info=True)
            return JsonResponse({"error": "Eligible quests not found."}, status=404)
        except ValidationError as e:
            logger.error(f"[COMPLETE QUEST] Validation error while serializing eligible quests for profile {profile.id}: {str(e)}", exc_info=True)
            return JsonResponse({"error": "Invalid data encountered during eligible quests serialization."}, status=400)
        except Exception as e:
            logger.error(f"[COMPLETE QUEST] General error while checking eligible quests for profile {profile.id}: {str(e)}", exc_info=True)
            return JsonResponse({"error": "An unexpected error occurred while checking eligible quests."}, status=500)
        ...
        try:
            ...
            logger.error(f"[COMPLETE QUEST] Error generating response for profile {profile.id}: {str(e)}", exc_info=True)
            return JsonResponse({"error": "An unexpected error occurred while generating the response."}, status=500)

    except OperationalError as e:
        # Handle database operational errors
        logger.error(f"[COMPLETE QUEST] Database operational error for profile {profile.id}: {str(e)}", exc_info=True)
        return JsonResponse({"error": "A database error occurred. Please try again later."}, status=500)
    except Exception as e:
        # Catch-all for unexpected errors
        logger.error(f"[COMPLETE QUEST] Error completing quest for user {profile.id}: {str(e)}", exc_info=True)
        return JsonResponse({"error": "An unexpected error occurred while completing quest"}, status=500)
...
def submit_bug_report(request):
    ...
    try:
        ...
        logger.error(f"[SUBMIT BUG REPORT] Failed to process bug report: {e}", exc_info=True)
